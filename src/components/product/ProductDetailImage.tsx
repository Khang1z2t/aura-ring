...
export function ProductDetailImage({ product }: ProductDetailImageProps) {
  ...
  return (
    <section data-track="pdp-image" className="rounded-[2rem] border border-border bg-card/60 p-4 md:p-6 lg:p-8">
      <div className="relative overflow-hidden rounded-[1.5rem] bg-surface">
        <div data-track="pdp-image-badges" className="absolute inset-x-0 top-0 z-10 flex items-center justify-between p-4 md:p-5">
          <p className="text-xs font-medium uppercase tracking-[0.22em] text-primary">{product.name}</p>
          {product.badge ? (
